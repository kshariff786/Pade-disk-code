...
real(8), dimension(sr:er, sphi:ephi, nz) :: uz, pressure, d_uz_dz
...
subroutine add_radial_derivatives (q, qdot)
...
real(8), dimension(sphi:ephi, sz_r:ez_r, nr):: pressure
...
if (isothermal) then
   do ir = 1, nr
      do iz = sz_r, ez_r
         do iphi = sphi, ephi
            pressure(iphi, iz, ir) = q_r_space(iphi, iz, irho, ir) * ci_squared_initial(ir, iz)
         end do
      end do
   end do
else
   do ir = 1, nr
      do iz = sz_r, ez_r
         do iphi = sphi, ephi
            pressure(iphi, iz, ir) = q_r_space(iphi, iz, ener, ir) * gm1 ! from internal energy
         end do
      end do
   end do
end if
...
if (apply_artificial_pressure) then
   ...
   do ir = 1, nr
      do iz = sz_r, ez_r
         do iphi = sphi, ephi
            pressure(iphi, iz, ir) = pressure(iphi, iz, ir) + p_art_r_space(iphi, iz, ir)
         end do
      end do
   end do
end if
...
do ir = 1, nr
   do iz = sz_r, ez_r
      do iphi = sphi, ephi
         ...
         F(iphi,iz,rmom,ir) = rgrid(ir)*(q_r_space(iphi,iz,rmom,ir)*ur(iphi,iz,ir) + pressure(iphi,iz,ir))
...
!      do iz = sz_r, ez_r
!         do iphi = sphi, ephi
!            F(iphi,iz,ener,ir) = (q_r_space(iphi,iz,ener,ir) + pressure(iphi,iz,ir)) * rgrid(ir) * ur(iphi,iz,ir)
!         end do
!      end do
!   end do
!end if
...
do ir = 1, nr
   do iz = sz_r, ez_r
      do iphi = sphi, ephi
         qdot_r_space(iphi,iz,rmom,ir) = qdot_r_space(iphi,iz,rmom,ir) + pressure(iphi,iz,ir)/rgrid(ir)
      end do
   end do
end do
      ...
      do iz = sz_r, ez_r
         do iphi = sphi, ephi
            qdot_r_space(iphi, iz, ener, ir) = qdot_r_space(iphi, iz, ener, ir) - &
                                       pressure(iphi, iz, ir)/rgrid(ir) * div_u_term(iphi, iz, ir)
         end do
      end do
   end do
end if
...
subroutine add_phi_derivatives(q, qdot, lambda_max_extra_operator)
...
#ifdef debug_print
   if (my_node .eq. 0) then
      print *, ' Calling transpose_z_to_phi for qdot in add_phi_derivatives'
   end if
#endif   
...
if (isothermal) then
   do iphi = 1, nphi
      do iz = sz_phi, ez_phi
         do ir = sr, er
            p(ir, iz, iphi) = q_phi_space(ir, iz, irho, iphi) * ci_squared_initial(ir, iz)
         end do
      end do
   end do
else
   do iphi = 1, nphi
      do iz = sz_phi, ez_phi
         do ir = sr, er
            p(ir, iz, iphi) = q_phi_space(ir, iz, ener, iphi) * gm1
         end do
      end do
   end do
end if
...
if (apply_artificial_pressure) then
   ...
   do iphi = 1, nphi
      do iz = sz_phi, ez_phi
         do ir = sr, er
            p(ir, iz, iphi) = p(ir, iz, iphi) + p_art_phi_space(ir, iz, iphi)
         end do
      end do
   end do
end if
...
if (apply_fargo_this_step) then
   ...
   do iphi = 1, nphi
      do iz = sz_phi, ez_phi
         do ir = sr, er
            ...
            F(ir, iz, amom, iphi) =  q_phi_space(ir, iz, amom, iphi) * uphi(ir, iz, iphi) / rgrid(ir)
            ...
            !c_sound = SQRT(gamma_sound_speed * p(ir, iz, iphi) / q_phi_space(ir, iz, irho, iphi))
            !lambda_max_local = (abs(uphi(ir,iz,iphi)) + c_sound) / (rgrid(ir)*dphi)
   ...
   if (.not. isothermal) then
      ...
      do iphi = 1, nphi
         do iz = sz_phi, ez_phi
            do ir = sr, er
               F(ir, iz, ener, iphi) = (q_phi_space(ir,iz,ener,iphi) + p(ir,iz,iphi)) * uphi(ir,iz,iphi)
            end do
         end do
      end do
   end if
      ...
      do iz = sz_phi, ez_phi
         do ir = sr, er
            qdot_phi_space(ir,iz,ener,iphi) = qdot_phi_space(ir,iz,ener,iphi) - &
                 p(ir,iz,iphi)/rgrid(ir)*div_u_term(ir,iz,iphi)
         end do
      end do
   end do
end if                           

! Phi pressure derivative for angular momentum eq.:
call pade_diff_periodic(mr*mz_phi, nphi, dphi, p(sr, sz_phi, 1), dp_dphi(sr, sz_phi, 1))

do iphi = 1, nphi
   do iz = sz_phi, ez_phi
      do ir = sr, er
         qdot_phi_space(ir,iz,amom,iphi) = qdot_phi_space(ir,iz,amom,iphi) - dp_dphi(ir, iz, iphi)
      end do
   end do
end do

if (add_fargo_extra_operator_now) then
   call add_fargo_extra_operator_terms (q_phi_space, qdot_phi_space, &
