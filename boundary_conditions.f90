...
implicit none
integer :: rmin_BC_arg, rmax_BC_arg, zmin_BC_arg, zmax_BC_arg, ibalanced_arg
real(8) :: Omega1_arg, Omega2_arg
...
if ((ibalanced .eq. 0) .or. (ibalanced .eq. 1)) then
   if (my_node .eq. 0) write(lun_info, *) ' ibalanced = ', ibalanced
else
   if (my_node .eq. 0) then
...
if (periodic_z .and. (zmin_BC .ne. periodic)) then
   write (lun_info, 3) zmin_BC
3  format (' Changed zmin_BC from ', i2, ' to periodic')
   zmin_BC = periodic
end if

if (periodic_z .and. (zmin_BC .ne. periodic)) then
   write (lun_info, 4) zmax_BC
...
if (my_node .eq. 0) then
   write (lun_info, 1) rmin_BC, rmax_BC, zmin_BC, zmax_BC
   1 format (' rmin_BC = ', i2,/, &
             ' rmax_BC = ', i2,/, &
             ' zmin_BC = ', i2,/, &
             ' zmax_BC = ', i2)

   write (lun_info, 2) null, non_reflective, zero_normal_momentum, &
...
subroutine specify_viscous_wall_conditions(Omega_rmin_arg, Omega_rmax_arg, &
