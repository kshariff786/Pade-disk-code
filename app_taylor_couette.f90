...
integer, parameter :: marcus = 1, dong = 2, axial = 3, meyer = 4, moser = 5, &
     check_viscous = 6, dong_counter_rotating = 7
! Sub-types:
...
use basic_state
! The use should not have to do this:
...
namelist /tc_input/ itype, isubtype, Re_Dong, restart, file_version, nr, nz, nphi, &
...
! Read namelist input:
if (my_node .eq. 0) then
   print *, ' node 0: about to open and read namelist file for VSI'      
...
if (my_node .eq. 0) print *, ' my_node = ', my_node, ' tau_filter = ', tau_filter
...
call specify_viscous_wall_conditions(Omega_i, Omega_o, uz_i, uz_o, c_sound_i, c_sound_o)
...
if (.not. restart) then
   ! The initial condition without perturbation is stored as the basic state.
   ! The basic state will be written into the restart file and
...
do istep = istep0 + 1, istep0 + nsteps
   ...
   if (negative_density) call terminate_with_save(1, istep, t)
   ! call print_max(q)
   if (my_node .eq. 0) then
   ...
   if (mod(istep - istep0, tecplot_interval) .eq. 0) then
      ...
      call tecplot_horizontal_plane (q, iz_mid, T_scale, t, istep)      
...
implicit none
integer, parameter :: nkr = 5, nkz = 5, nkphi_dim = 8
integer :: nkphi
...
subroutine output_uphi_versus_r(t, lun)
...
write(filename, "('uphi_t_', i6.6, f0.4, '.dat')") int(t), t - int(t)
open (unit = lun, file = filename, form = 'formatted', status = 'unknown')

do ir = 1, nr
   write(lun, "(3(1x,e12.5))") rgrid(ir), q(ir, 1, iz_mid, amom)/q(ir, 1, iz_mid, irho)/rgrid(ir), &
