...
module control_parameters
   implicit none
   integer :: istep, nsteps, n_words_allocated
   real(8) :: t, cfl
...
module dof_indices
   ...
   integer, parameter :: iuz = 2, iur = 3, iuphi = 4, ip = 5
...
module grid
   ...
   ! Grid spacing vector for Vreman model:
   ...
   ! Useful for outputting the midplane:
   integer :: iz_mid, ir_mid
end module grid   

module gravity
   ! This is for an axisymmetric (r, z) gravity due to a point mass or
   ! a finite sphere of radius R_sphere.
   implicit none
   integer :: gravity_flag
   logical :: finite_sphere
   real(8) :: GM, R_sphere
   ! Radial and vertical gravitational acceleration
   ! Gravitational potential energy Phi_g = -GM/R.  We are defining it so it
   ! is higher away from the point mass.
   ! Dimensions of gr and gz are (nr, nz).
   real(8), allocatable, dimension(:, :) :: gr, gz, Phi_g
...
module logical_units
   ...
   integer, parameter :: lun_info = 20 ! Information file info.txt
...
module math_constants
   implicit none
   real(8) :: pi ! Given its value in domain_and_mesh_partition.f90
