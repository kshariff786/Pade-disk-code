...
#ifdef debug_print
   if (my_node .eq. 0) print *, ' node = 0: set_up_domain_mesh_and_partition: About to open info.txt'
#endif

if (my_node .eq. 0) then
   ! We don't begin with access = 'append' because we want to clear any previous
   ! info file:
   open(unit = lun_info, file = 'info.txt', form = 'formatted', status = 'unknown')
   write(lun_info, "(' nr = ', i4, ' nz = ', i4, ' nphi = ', i4)") nr, nz, nphi
end if

#ifdef debug_print
   if (my_node .eq. 0) print *, ' node = 0: set_up_domain_mesh_and_partition: opened info.txt'
#endif
...
#ifdef mpi_code
   if (my_node .eq. 0) then
      write(lun_info, "(' mpi run with num_nodes = ', i5)") num_nodes
      write(lun_info, "(' cpu layout: ng1 = ', i4, ' ng2 = ', i4)") ng1, ng2
   end if
#endif   
...
n_words_allocated = n_words_allocated + 2*mphi*nz*ndof*nr
...
n_words_allocated = n_words_allocated + 2*mr*mz_phi*ndof*nphi
#ifdef debug_print
...
#ifdef debug_print
   if (my_node .eq. 0) print *, ' my_node = 0: set_up_domain_mesh_and_partition: Returned from get_xy_coordinates_of_grid'
#endif
...
if (my_node .eq. 0) close(lun_info)
