...
module fargo_or_plotting_shift
   ...
   real(8), allocatable, dimension(:)   :: fargo_factor   
...
subroutine setup_Omega_bar_for_fargo(igravity, GM)

use grid
use fargo_or_plotting_shift
use control_parameters
implicit none
integer, intent(in) :: igravity
real(8), intent(in) :: GM
...
do iphi = 1, nphi
   do iz = sz_phi, ez_phi
      do ir = sr, er
         ...
         F(ir, iz, rmom, iphi) =  q(ir, iz, rmom, iphi) * rgrid(ir) * ur
      ...
      do iz = sz_phi, ez_phi
         do ir = sr, er
            qdot(ir, iz, idof, iphi) = qdot(ir, iz, idof, iphi) - &
                                         dF(ir, iz, idof, iphi) * fargo_factor(ir)
         end do
      end do
   end do
end do
   ...
   do iz = sz_phi, ez_phi
      do ir = sr, er
         qdot(ir, iz, ener, iphi) = qdot(ir, iz, ener, iphi) - &
                                    p(ir, iz, iphi) * dF(ir, iz, div_u, iphi) * fargo_factor(ir)
      end do
   end do
end do
...
do iphi = 1, nphi
   do iz = sz_phi, ez_phi
      do ir = sr, er
         qdot(ir,iz,rmom,iphi) = qdot(ir,iz,rmom,iphi) - dp_dphi(ir,iz,iphi) * fargo_factor(ir)
      end do
   end do
end do
