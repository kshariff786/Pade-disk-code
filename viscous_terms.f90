...
subroutine strain_tensor(q)   ! subtract_basic_state_uphi)
...
! --------------
if (nr .ne. 1) then
   ! Item 4: 0.5*duz/dr.  Fargo is done in item 12 below and then
   ...
   T_zr = deriv_in_z_space ! T_zr partial
   ...
   ! Item 6: 0.5*duphi/dr = First term of T_pr.  Its Fargo is done in item 10 below.
...
if (nphi .ne. 1) then
   ...
   ! Item 12:  Adjust item 4 with Fargo since we have duz/dphi available:
   if (add_fargo_extra_operator_now) then
      do ir = sr, er
...
if ((nz .ne. 1) .and. (.not. suppress_z_derivatives_when_nz_not_1)) then
   ...
   T_zr = 0.5d0*(T_zr + deriv_in_z_space)
