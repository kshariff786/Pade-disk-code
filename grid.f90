...
logical :: planar_case, case1, case2
...
if ((nz .ne. 1) .and. (.not. suppress_z_derivatives_when_nz_not_1) .and. (nr .ne. 1) .and. (nphi .ne. 1)) then
   ...
   ! Grid spacing vector:
   if (my_node .eq. 0) write(lun_info, "('Computed min_grid_size for 3D case')")
   ...
   write(lun_info, "('Computed min_grid_size for planar case')")
   ...
   if (my_node .eq. 0) write(lun_info, "('Computed min_grid_size for axisymmetric case')")   
...
! Output:
if (my_node .eq. 0) then
   print *, ' writing to lun_info in stretched_mesh_r'
   write(lun_info, "(' stretch_r        = ', e12.5)") stretch_r
   write(lun_info, "(' savings_factor_r = ', e12.5)") savings_factor_r
...
! Output:
if (my_node .eq. 0) then
   write(lun_info, "(' stretch_z        = ', e12.5)") stretch_z
   write(lun_info, "(' savings_factor_z = ', e12.5)") savings_factor_z
