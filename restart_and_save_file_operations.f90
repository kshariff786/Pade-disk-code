...
write (filename_given, "('save_', i7.7)") istep
...
allocate(uphi_basic_r_space(sz_r:ez_r, nr)) ! needed for vorticity computation
...
! Note: The filename given to save file is output to the caller.
...
if (ier/=MPI_SUCCESS) then
   write(6,*) "Can't MPI_FILE_OPEN the restart file mpi_restart"
   call MPI_ABORT(MPI_COMM_WORLD, 99, ier)
end if
