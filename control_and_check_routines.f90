...
subroutine stop_if_negative_density(q)
   ...
   do iphi = sphi, ephi
      do ir = sr, er
         if (q(ir, iphi, iz, irho) .le. 0.d0) then
            print *, ' In stop_if_negative_density, density is <= 0'
...
end subroutine stop_if_negative_density

!----------------------------------------------------------------------------------85

subroutine check_for_negative_density(q, sub_step, negative_density)
...
! Local
integer :: ier, ir, iphi, iz
logical :: i_have_a_negative_density ! this processor 

i_have_a_negative_density = .false.
negative_density          = .false.
do iz = 1, nz
   do iphi = sphi, ephi
      do ir = sr, er
         if (q(ir, iphi, iz, irho) .lt. 0.0d0) then
            print *, ' density is negative at sub-step = ', sub_step
            print *, ' ir = ', ir, ' iz = ', iz, ' iphi = ', iphi
            open (unit = lun_info, file = 'info.txt', form = 'formatted', &
                 status = 'unknown', access = 'append')
            write(lun_info, "(' density is negative at sub-step = ', i1)") sub_step
            write(lun_info, "(' ir = ', i4, ' iz = ', i4, ' iphi = ', i4)") ir, iz, iphi
            close(lun_info)
            i_have_a_negative_density = .true.
         end if
      end do   
   end do
end do
...
#ifdef mpi_code
   call mpi_allreduce(i_have_a_negative_density, negative_density, 1, &
                      mpi_logical, mpi_lor, mpi_comm_world, ier)
#else
   negative_density = i_have_a_negative_density
#endif

end subroutine check_for_negative_density
...
if (my_node .eq. 0) then
   ...
   open(unit = lun_general_purpose_1, file = 'name_of_last_save_file.dat', form = 'formatted', &
        status = 'unknown')
