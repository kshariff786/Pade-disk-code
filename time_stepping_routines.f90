...
subroutine rk4(rhs, q, cfl, t, dt, use_supplied_dt, negative_density)
...
logical, intent(out)   :: negative_density
...
#ifdef debug_print
   if (my_node .eq. 0) print *, ' node 0: calling add_sponge_term'
#endif
   call add_sponge_term(dt, q, qdot)
#ifdef debug_print
   if (my_node .eq. 0) print *, ' node 0: returned from add_sponge_term'
#endif   
end if

q_next_arg = q + 0.5d0    *dt*qdot ! arg for k2
!call stop_if_negative_density(q_next_arg)
call check_for_negative_density(q_next_arg, 1, negative_density)
if (negative_density) return
...
if (apply_fargo_this_step) then
   ...
   if (my_node .eq. 0) print *, ' Not applying FARGO this step'
end if
...
first_substep = .false.
! gives k2/dt  
call rhs(q_next_arg, qdot, first_substep, t + 0.5d0*dt, t0, lambda_max, lambda_max_extra_operator)
if (apply_sponge) call add_sponge_term(dt, q_next_arg, qdot)
q_next_arg = q       +     0.5d0*dt*qdot ! arg for k3
!call stop_if_negative_density(q_next_arg)
call check_for_negative_density(q_next_arg, 2, negative_density)
if (negative_density) return
...
!call stop_if_negative_density(q_next_arg)
call check_for_negative_density(q_next_arg, 3, negative_density)
if (negative_density) return
q_accum    = q_accum + 1.d0/3.d0*dt*qdot
...
! gives k4/h 
call rhs(q_next_arg, qdot, first_substep, t + dt, t0, lambda_max, lambda_max_extra_operator)
if (apply_sponge) call add_sponge_term(dt, q_next_arg, qdot)
q = q_accum + 1.d0/6.d0*dt*qdot
!call stop_if_negative_density(q)
call check_for_negative_density(q, 4, negative_density)
if (negative_density) return
...
subroutine rk4_with_target(rhs, q, cfl, t, dt, use_supplied_dt, negative_density, &
...
! If use_supplied_dt is true we will ignore the given "dt" and will choose "dt" to hit
! the target.
...
#endif
first_substep = .true.
t0            = t      ! t0 is for use by Fargo (if we do it).
call rhs(q, qdot, first_substep, t, t0, lambda_max, lambda_max_extra_operator)
...
q_next_arg = q + 0.5d0    *dt*qdot ! arg for k2
call check_for_negative_density(q_next_arg, 1, negative_density)
if (negative_density) call terminate_with_no_save(1)
q_accum    = q + 1.d0/6.d0*dt*qdot
...
! We don't need to add the Fargo extra operator outside of time-stepping.
! For example, we don't need it when doing diagnostics.
add_fargo_extra_operator_now = .false.
...
end subroutine rk4_with_target
...
if (my_node .eq. 0) print *, ' node 0: In get_lambda_euler_non_fargo, starting loop for isothermal'
   ...
   do iz = 1, nz
      do iphi = sphi, ephi
         do ir = sr, er
#ifdef debug_print
if (my_node .eq. 0) print *, ' node 0: ir = ', ir, ' iphi = ', iphi, ' iz = ', iz
#endif            
            rho = q(ir, iphi, iz, irho)
#ifdef debug_print
            if (my_node .eq. 0) then
               print *, ' ci_squared_initial(ir, iz) = ', ci_squared_initial(ir, iz)
               print *, ' rho = ', rho
               print *, ' rgrid(ir) = ', rgrid(ir)
               print *, ' dphi = ', dphi
            end if
#endif
            c = SQRT(ci_squared_initial(ir, iz))
