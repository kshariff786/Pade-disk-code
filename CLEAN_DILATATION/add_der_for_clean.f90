...
subroutine add_z_derivatives_for_clean_dilatation(q, qdot)
...
return
end subroutine add_z_derivatives_for_clean_dilatation

!----------------------------------------------------------------------------------85

subroutine add_radial_derivatives_for_clean_dilatation(q, qdot)
...
real(8) :: rho_2_q2, rho_q2
   ...
   if (my_node .eq. 0) then
      print *, ' add_radial_derivatives_for_clean_dilatation has been called'
   end if
#endif
...
do ir = 1, nr
   do iz = sz_r, ez_r
      do iphi = sphi, ephi
         F(iphi,iz,rmom,ir) = rgrid(ir)*(p_art_r_space(iphi,iz,ir))
      end do
   end do
end do
...
do ir = 1, nr
   do iz = sz_r, ez_r
      do iphi = sphi, ephi
         qdot_r_space(iphi,iz,rmom,ir) = qdot_r_space(iphi,iz,rmom,ir) + p_art_r_space(iphi,iz,ir)/rgrid(ir)
      end do
   end do
end do
